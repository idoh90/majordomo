  ...
  privacy: {
    ...
    sections: [
      ...
      },
      {
        h: 'Google Calendar',
        p: [
          'Connecting Google Calendar is optional, and it stays off until you ask for it. When you connect, Majordomo is granted exactly two permissions: it may create and manage a calendar of its own inside your Google account, and it may read the events on your calendars. It can never edit or delete an event you made yourself.',
          'What crosses is what you can see it doing. Your Majordomo bookings are written into a calendar named “Majordomo”, so your phone and your Apple or Google calendar apps can show them; your own Google events are drawn on the Manor week as read-only blocks, so nothing is booked on top of a real commitment. Titles and times cross. Notes and descriptions do not, in either direction.',
          'The key that keeps the connection lives on our server, never in your browser, and only the server can read it. Google Calendar data is kept for as long as you stay connected, and is used only to provide the calendar features described here.',
          'Majordomo’s use of information received from Google APIs adheres to the Google API Services User Data Policy, including its Limited Use requirements. Google Calendar data is never sold or transferred, never used for advertising, never used to train any model — general or personalised — and never read by a human except with your permission or where the law requires it.',
          'Disconnect at any time from Settings, under Calendars: Majordomo forgets the key, asks Google to revoke it, and removes the imported events from the Manor. You can also revoke access from your Google account, and you can delete the “Majordomo” calendar there without touching anything else.',
        ],
      },
      {
